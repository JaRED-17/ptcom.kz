import { defineMessages } from 'react-intl'

export default defineMessages({
  'app.page.aboutUs.title': {
    id: 'app.page.aboutUs.title',
    defaultMessage: 'О компании'
  },
  'app.page.staffs.title': {
    id: 'app.page.staffs.title',
    defaultMessage: 'Сотрудники'
  },
  'app.page.news.title': {
    id: 'app.page.news.title',
    defaultMessage: 'Новости'
  },
  'app.page.licenses.title': {
    id: 'app.page.licenses.title',
    defaultMessage: 'Лицензии'
  },
  'app.page.vacancies.title': {
    id: 'app.page.vacancies.title',
    defaultMessage: 'Вакансии'
  },
  'app.page.services.title': {
    id: 'app.page.services.title',
    defaultMessage: 'Наши услуги'
  },
  'app.page.provisionOfWagons.title': {
    id: 'app.page.provisionOfWagons.title',
    defaultMessage: 'Предоставление вагонов'
  },
  'app.page.paymentRailwayTariffs.title': {
    id: 'app.page.paymentRailwayTariffs.title',
    defaultMessage: 'Оплата ЖД тарифов'
  },
  'app.page.driveWays.title': {
    id: 'app.page.driveWays.title',
    defaultMessage: 'Подъездные пути'
  },
  'app.page.schema.title': {
    id: 'app.page.schema.title',
    defaultMessage: 'Разработка и согласование схем погрузки'
  },
  'app.page.handlingDostyk.title': {
    id: 'app.page.handlingDostyk.title',
    defaultMessage: 'Перегруз груза на ст. Достык'
  },
  'app.page.cargoProcessing.title': {
    id: 'app.page.cargoProcessing.title',
    defaultMessage: 'Грузопереработка'
  },
  'app.page.consultation.title': {
    id: 'app.page.consultation.title',
    defaultMessage: 'Консультации'
  },
  'app.page.dangerousGoods.title': {
    id: 'app.page.dangerousGoods.title',
    defaultMessage: 'Классификации опасных грузов на железнодорожном транспорте'
  },
  'app.page.containers.title': {
    id: 'app.page.containers.title',
    defaultMessage: 'Основные типы контейнеров'
  },
  'app.page.wagons.title': {
    id: 'app.page.wagons.title',
    defaultMessage: 'Общие сведения об устройстве вагонов'
  },
  'app.page.wagonsTypes.title': {
    id: 'app.page.wagonsTypes.title',
    defaultMessage: 'Основные типы вагонов'
  },
  'app.page.calculation.title': {
    id: 'app.page.calculation.title',
    defaultMessage: 'Расчет тарифов'
  },
  'app.page.contacts.title': {
    id: 'app.page.contacts.title',
    defaultMessage: 'Контакты'
  },
  'app.page.sitemap.title': {
    id: 'app.page.sitemap.title',
    defaultMessage: 'Карта сайта'
  },
  'app.page.pageNotFound.title': {
    id: 'app.page.pageNotFound.title',
    defaultMessage: 'Страница не найдена'
  },
  'app.page.stRkIso90012009.title': {
    id: 'app.page.stRkIso90012009.title',
    defaultMessage: 'СТ РК ИСО 9001–2009'
  },
  'app.page.5years.title': {
    id: 'app.page.5years.title',
    defaultMessage: 'Наша компания - 5 лет на рынке!'
  },
  'app.page.tl2015.title': {
    id: 'app.page.tl2015.title',
    defaultMessage: 'Участие в выставке «Транспорт и Логистика», г. Алматы.'
  },
  'app.page.tr2015.title': {
    id: 'app.page.tr2015.title',
    defaultMessage: 'Участие в выставке «TransRussia 2015», г. Москва, МВЦ «Крокус Экспо'
  },
  'app.page.iso.title': {
    id: 'app.page.iso.title',
    defaultMessage: 'Сертификация ISO 9001:2009'
  },
  'app.page.transit.title': {
    id: 'app.page.transit.title',
    defaultMessage: 'Участие в 18-й Центрально-Азиатской выставке Transit-Kazakhstan, «Транспорт и логистика»'
  },
  'app.page.crocus.title': {
    id: 'app.page.crocus.title',
    defaultMessage: 'Участие в 19-й международной выставки TRANS RUSSIА, 22-25 апреля 2014 года, Москва, CROCUC EXPO'
  },
  'app.page.zvezda2015.title': {
    id: 'app.page.zvezda2015.title',
    defaultMessage: 'Получена новая награда «Звезда качества 2015»'
  },
  'app.page.staticPage.title': {
    id: 'app.page.staticPage.title',
    defaultMessage: 'HTML контент'
  }
})